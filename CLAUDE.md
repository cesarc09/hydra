...
- **Token usage: stable row identity is the correctness boundary; offsets are only an optimisation.** `usage_messages.message_id` is the PK and ordinary ingest is `INSERT OR IGNORE`, which makes Stop-hook retries, `usage backfill` re-runs, Codex sweeps and resumed sessions safe. Claude rows use `message.id`; Codex rows use a deterministic content key. The client's byte offsets only decide what is *sent* and may be lost or stale without corrupting anything. `usage reconcile codex` is the explicit exception: it reparses from byte zero, previews every chunk, and only `--apply` replaces parser-derived fields on same-instance Codex rows while preserving provenance and sweep state. Traps that shaped this, all measured on a 719-file / 41k-record Claude corpus:
  ...
  - **Codex usage is cumulative and must be differenced.** In the measured rollout corpus, 121 events re-emitted a prior cumulative total with non-zero `last_token_usage`, while every reset started clean. The parser therefore diffs cumulative counters, uses `last_token_usage` when a counter decreases or a changed cumulative snapshot equals the latest call, drops zero deltas, and keys rows as `(thread, timestamp, cumulative total)`. The shared sweep state stores one byte offset per file, skips unchanged files by size without opening them, and safely replays after lost state through `INSERT OR IGNORE`. Guardian review rows inherit the immediate parent's model at the child's start time. Stop requests 10 s; SessionEnd requests Codex's 3 s clamp and remains best-effort.
